...
Public Class MainForm
  ...
  Private Sub LoadCategories()
    Try
      ...
      For Each strCategory As String In Directory.GetDirectories(m_strPath + "Categories")
        LoadCategoryFolder(strCategory, Nothing)
  ...
  Private Sub LoadCategoryFolder(ByVal strCategory As String, parentNode As TreeNode)

    Dim trvCategoryFolder As New TreeNode(New DirectoryInfo(strCategory).Name)
    Dim objCategoryFolderTag As New Categories.Tags.CategoryFolder(strCategory)
  ...
  Private Sub SelectTreeNode()
    Try
      ...
      ElseIf TypeOf (trvMain.SelectedNode.Tag) Is Categories.Tags.CategoryFolder Then
        ...
        m_objCategoryListControl.CategoryFile = CType(trvMain.SelectedNode.Tag, Categories.Tags.CategoryFolder).Path
  ...
  Private Sub CreateNewCategoryFolder()
  ...
  Private Sub CreateNewCategoryItem()
    Dim strCategory As String = ""
  ...
  Private Sub CreateNewCategory(ByVal Path As String, Optional ByVal Name As String = "")
    Try
      ...
      Dim objCategoryTag As New Categories.Tags.CategoryFile(strFileName, Name)
      trvCategoryItem.Tag = objCategoryTag
      trvCategoryItem.ImageIndex = 16
      trvCategoryItem.SelectedImageIndex = 16

      ' Add Item
      Dim relativePath As String() = Path.Replace(m_strPath + "Categories", "").Split("\")
      Dim parentNode As TreeNode = trvMain.Nodes(1)
      For Each categoryFolder As String In relativePath
    ...
    Try
      ...
      ElseIf TypeOf (trvMain.SelectedNode.Tag) Is Categories.Tags.CategoryFolder Then
        ...
        If m_objCategoryListControl.txtName.Text <> trvMain.SelectedNode.Text Then
          ...
          CType(trvMain.SelectedNode.Tag, Categories.Tags.CategoryFolder).Path = m_objCategoryListControl.CategoryFile
        End If
      ElseIf TypeOf (trvMain.SelectedNode.Tag) Is Categories.Tags.CategoryFile Then
        m_objCategoryViewControl.Save()
  ...
  Private Sub SearchCategoryFileSelected(ByVal CategoryItem As String)
    Dim relativePath As String() = CategoryItem.Replace(m_strPath + "Categories", "").Split("\")
    Dim parentNode As TreeNode = trvMain.Nodes(1)
    For Each categoryFolder As String In relativePath
      If Not categoryFolder = "" Then
  ...
  Private Sub toolCategories_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
  ...
  Private Sub ReloadDreamNodes(ByVal trvNode As TreeNode)
    ...
    For Each XmlNode As XmlNode In xmlDoc.DocumentElement.SelectNodes("Categories")
      If XmlNode.Name <> "Unassigned" Then
        Dim trvCategory As New TreeNode(XmlNode.InnerText)
        trvCategory.Tag = New Categories.Tags.CategoryFile(m_strPath & "Categories\" & XmlNode.Name & "\" & XmlNode.InnerText & ".ld3", XmlNode.Name)
        trvCategory.ImageIndex = 16
        trvCategory.SelectedImageIndex = 16
    ...
    For Each XmlNode As XmlNode In xmlDoc.DocumentElement.SelectNodes("SubCategories/Category")
      Dim trvCategory As New TreeNode(XmlNode.InnerText)
      trvCategory.Tag = New Categories.Tags.CategoryFile(m_strPath & "Categories\" & XmlNode.Attributes("Path").InnerText & "\" & XmlNode.InnerText & ".ld3", XmlNode.Attributes("Path").InnerText)
      trvCategory.ImageIndex = 16
      trvCategory.SelectedImageIndex = 16
      trvNode.Nodes.Add(trvCategory)
  ...
  Private Sub NewToolStripMenuItem_DropDownOpening(sender As System.Object, e As System.EventArgs) Handles NewToolStripMenuItem.DropDownOpening
